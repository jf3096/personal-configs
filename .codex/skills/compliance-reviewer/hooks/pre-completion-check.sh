...
DELIVERY_GATE_SCRIPT="/home/jf3096/.codex/skills/compliance-reviewer/scripts/delivery_gate.py"
...
if [ "$CLAIMS_RESULT" = true ]; then
  if ! printf '%s\n' "$RESPONSE" | COMPLIANCE_REVIEW_VERBOSE="$VERBOSE" python3 "$DELIVERY_GATE_SCRIPT"; then
    exit 1
  fi
