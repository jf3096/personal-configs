...
from conftest import compliance_receipt_block, run_hook, stage2_executable_recheck_block, verdict_block
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            compliance_receipt_block() + verdict_block() + '状态：完成（已验证：pytest -q 全部通过）\n',
            0,
            '',
        ),
        (
        ...
        (
            ...
            compliance_receipt_block() + verdict_block() + '本次改动 should work。\n状态：完成（已验证：pytest -q）\n',
            1,
...
def test_verbose_probe(global_hook_path) -> None:
    body = compliance_receipt_block() + verdict_block() + '状态：完成（已验证：pytest -q 全部通过）\n'
